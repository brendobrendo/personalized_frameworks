...
function VoiceScreen({ navigation, route }: any) {
    ...
    const processAudio = async (audioUri: string) => {
        try {
            ...
            const response = await fetch(`data:audio/m4a;base64,${audioData}`);
            ...
            // Optionally POST the transcription somewhere
            try {
                const uploadResponse = await fetch('https://e18dq9d24d.execute-api.us-west-2.amazonaws.com/prod/upload', {
                    method: 'POST',
                ...
                console.error('Failed to upload transcription:', err);
            }
        } catch (err) {
    ...
    if (hasPermission === null) {
        return <View style={cameraStyles.container}><ActivityIndicator size="large" /></View>;
    }
    
    if (hasPermission === false) {
        return <View style={cameraStyles.container}><Text>No access to microphone</Text></View>;
    }
            ...
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 }}>
                {isRecording && (
                    <View style={{ marginBottom: 30 }}>
                        <Text style={{ fontSize: 24, color: '#ff0000', fontWeight: 'bold' }}>
                            ● Recording...
                        </Text>
                    </View>
                )}
                ...
                {transcription !== '' && !isRecording && (
                    <View style={{ 
                        ...
                        <Text style={{ fontSize: 14 }}>{transcription}</Text>
                    </View>
                )}

                <TouchableOpacity 
                    style={[
                        cameraStyles.cameraButton,
                        { 
                            position: 'relative',
                            backgroundColor: isRecording ? '#ff0000' : '#007AFF'
                        }
                    ]} 
