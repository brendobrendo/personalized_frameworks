...
## Postgres table ##
The `wells_fargo_credit_card_transactions` table lives in the `public` schema and stores individual credit card transactions from Wells Fargo. Each row is uniquely identified by a non-nullable `id` of type `uuid`, generated by default with `gen_random_uuid()`. Transactions have a required `transaction_date` (`date`) and a required `amount` (`numeric(10,2)`), with a check constraint enforcing that the amount is non-zero. Optional metadata includes a one-character `posted_flag`, a free-form `raw_reference`, and optional foreign keys `merchant_id` and `category_id` (both `uuid`) used for normalization. The unnormalized merchant name from the source system is stored in the required `raw_merchant` (`text`) column. Audit fields `created_at` and `updated_at` are both non-nullable `timestamp with time zone` values defaulting to `now()`. The table uses a heap access method, has a primary-key btree index on `id`, and is referenced by the `receipts` table via the `fk_receipt_credit_tx` foreign key, which sets the reference to `NULL` on delete.
## Flask code ##
